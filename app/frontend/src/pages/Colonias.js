...
const Colonias = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await api.post("/api/colonias/colonias/", formData);
      setMensaje("Colonia creada exitosamente");
      setColonias([...colonias, response.data]);
      setFormData({
        nombre: "",
        ubicacion: "",
        responsable_voluntario: "",
      ...
      const mensaje = error?.response?.data?.detail || "Error al crear la colonia";
      setMensaje(mensaje);
    }
  };
  ...
  return (
    <div className="container mt-4">
      ...
      {/* Formulario para crear una nueva colonia */}
      <div className="card mb-4">
        <div className="card-body">
          <h3>Crear Colonia</h3>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-3">
              <label htmlFor="responsable_voluntario" className="form-label">Responsable Voluntario</label>
              <input
                type="text"
                className="form-control"
                id="responsable_voluntario"
                name="responsable_voluntario"
                value={formData.responsable_voluntario}
                onChange={handleInputChange}
              />
            </div>
            <button type="submit" className="btn btn-primary">Crear Colonia</button>
          </form>
        </div>
      </div>
      <AsignarUsuarioColonia />
    </div>
  );
};
...
// 🔗 NUEVA SECCIÓN: Asignar Usuario a Colonia

const AsignarUsuarioColonia = () => {
  ...
  useEffect(() => {
    api.get("/api/voluntarios/usuarios_asignables/").then((res) => setUsuarios(res.data));
    api.get("/api/colonias/colonias/").then((res) => setColonias(res.data));
  }, []);

  const asignar = () => {
      api
        .post("/api/colonias/asignar_usuario/", {
          user_id: parseInt(userId),
          colonia_id: parseInt(coloniaId),
        })
        .then(() => setMensaje("Usuario asignado correctamente ✅"))
        .catch(() => setMensaje("Error al asignar usuario ❌"));
    };

  return (
    <div className="container mt-5 mb-5 p-4 border rounded bg-light">
      <h4>Asignar Usuario a Colonia</h4>
      <div className="form-group">
        <label>Usuario/Voluntario:</label>
      ...
      <div className="form-group mt-3">
        <label>Colonia:</label>
        <select className="form-control" value={coloniaId} onChange={(e) => setColoniaId(e.target.value)}>
      ...
      <button className="btn btn-primary mt-4" onClick={asignar} disabled={!userId || !coloniaId}>
        Asignar
      </button>
